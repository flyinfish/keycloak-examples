...
## scenarios
there are 3 clients `service-1-client`, `service-2-client` and `service-3-client`.

### token-exchange starting with grant_type=client_credentials
we try
...
```
  ...
  echo $token1  
``` 
2. then exchange for `service-2-client` using `service-1-client`
```
  ...
  echo $token2  
``` 
3. then exchange for `service-3-client` using `service-2-client`<br>unfortunately this fails with `Http 400 Bad Request - invalid_token`<br>logging `DEBUG [org.keycloak.services.managers.AuthenticationManager] (executor-thread-25) Client session for client 'service-1-client' not present in user session 'bcc55f7b-bf6a-488b-99af-f46cf4e8ef6c'`
```
  curl -s -i -X POST \
    --location http://localhost:8881/realms/dev/protocol/openid-connect/token \
    --header "Content-Type: application/x-www-form-urlencoded" \
...
### token-exchange starting with grant_type=password
we try
1.  get a token for `service-1-client` using client-credentials<br>
```
  token1=$(curl -s -X POST \
